...
```
逻辑处理肯定就不能放在unity的update里面啦，这里定义`IHasGameFrame`的接口，凡是需要以同步帧驱动的逻辑，实现这个接口并且将其添加到上面的`GameFrameLogicObjList`逻辑帧对象列表中，在上面的逻辑帧中也就会处理该接口的逻辑了。
```csharp
public interface IHasGameFrame
{
    void GameFrameTurn(int gameFrameTime);
